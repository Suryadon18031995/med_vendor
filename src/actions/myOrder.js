...
export const receiveMyOrderData = data => ({
  ...
  return dispatch => {
    ...
    axios.get(MY_ORDER_CONSTANTS.MY_ORDER_URL+'?searchCriteria[filterGroups][0][filters][0][field]=customer_id&searchCriteria[filterGroups][0][filters][0][value]='+data+'&searchCriteria[sortOrders][0][field]=entity_id&searchCriteria[sortOrders][0][direction]=DESC',
    {  
        headers: 
        {
      ...
      .then(res => dispatch(receiveMyOrderData(res.data)))
...
export const receiveMyOrderDataById = data => ({
  ...
  export const fetchMyOrderDataById = (data,data1) => {
      console.log(data,data1);
    return dispatch => {
      dispatch(requestMyOrderDataById());
      axios.get(MY_ORDER_CONSTANTS.ORDER_ID_URL+'?searchCriteria[filterGroups][0][filters][0][field]=entity_id&searchCriteria[filterGroups][0][filters][0][value]='+data+'&searchCriteria[sortOrders][0][field]=entity_id&searchCriteria[sortOrders][0][direction]=DESC&fields=items[entity_id,increment_id,customer_id,subtotal,discount_amount,tax_amount,shipping_amount,grand_total,created_at,customer_firstname,customer_lastname,status,shipping_amount,shipping_description,coupon_code,items[name,qty_ordered,discount_amount,row_total,sku,price,extension_attributes],billing_address,extension_attributes[shipping_assignments[shipping[address]],payment_details]]',
      {  
          headers: 
          {
              'Content-Type': 'application/json',
