...
class ViewOrderContainer extends Component {
    ...
    componentDidMount() {
        ...
        this.props.getViewOrderData(this.props.history.location.state.orderId,this.props.apiToken);
    }

    UNSAFE_componentWillReceiveProps(nextProps) {
          console.log(nextProps.orderDetails);
        if (!_isEmpty(_get(nextProps, 'orderDetails'))) {
                const orderDate = moment(_get(nextProps.orderDetails.items[0].created_at)).format('MMMM Do YYYY');
                ...
                this.setState({
                    ...
                    orderDate,
                    orderNumber: nextProps.orderDetails.items[0].entity_id,
                    //mapDetails: _get(nextProps.viewOrderData, [0, 'map_details']),
                    status: nextProps.orderDetails.items[0].status,
                    shippingAddress: nextProps.orderDetails.items[0].extension_attributes.shipping_assignments[0].shipping,
                    billingAddress: nextProps.orderDetails.items[0].billing_address,
                    paymentMethod: nextProps.orderDetails.items[0].extension_attributes.payment_details,
...
const mapDispatchToProps = dispatch => ({
    getViewOrderData: (data,data1) => dispatch(fetchMyOrderDataById(data,data1)),
