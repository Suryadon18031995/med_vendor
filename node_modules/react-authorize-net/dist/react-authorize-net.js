import _extends from '@babel/runtime/helpers/extends';
import _objectWithoutPropertiesLoose from '@babel/runtime/helpers/objectWithoutPropertiesLoose';
import React, { useEffect, useState } from 'react';
import MaskedInput from 'react-text-mask';
import { Box, Flex, Text } from 'rebass';
import _taggedTemplateLiteralLoose from '@babel/runtime/helpers/taggedTemplateLiteralLoose';
import { values, toPairs, curry, head, pick } from 'ramda';
import { FaCreditCard, FaExclamationCircle, FaRegCalendarAlt } from 'react-icons/fa';
import { Motion, spring } from 'react-motion';
import styled, { createGlobalStyle } from 'styled-components';
import _regeneratorRuntime from '@babel/runtime/regenerator';
import _asyncToGenerator from '@babel/runtime/helpers/asyncToGenerator';
import _assertThisInitialized from '@babel/runtime/helpers/assertThisInitialized';
import _inheritsLoose from '@babel/runtime/helpers/inheritsLoose';
import PropTypes from 'prop-types';
import { parse } from 'query-string';

var robotoFont = "data:font/woff2;base64,";

var Input = function Input(props) {
  var onChange = props.onChange,
      onFocus = props.onFocus,
      onBlur = props.onBlur,
      mask = props.mask,
      valid = props.valid,
      focused = props.focused,
      label = props.label,
      className = props.className,
      style = props.style,
      otherProps = _objectWithoutPropertiesLoose(props, ["onChange", "onFocus", "onBlur", "mask", "valid", "focused", "label", "className", "style"]);

  var onChangeHandler = function onChangeHandler(ev) {
    ev.persist();

    if (onChange) {
      onChange(ev);
    }
  };

  var onFocusHandler = function onFocusHandler(ev) {
    ev.persist();

    if (onFocus) {
      onFocus(ev);
    }
  };

  var onBlurHandler = function onBlurHandler(ev) {
    ev.persist();

    if (onBlur) {
      onBlur(ev);
    }
  };

  return React.createElement(Flex, {
    className: className,
    alignItems: "center",
    style: style
  }, props.label ? React.createElement(Flex, {
    flexDirection: 'row-reverse',
    mr: 2
  }, props.label) : null, React.createElement(Box, null, React.createElement(MaskedInput, _extends({
    mask: mask,
    onChange: onChangeHandler,
    onFocus: onFocusHandler,
    onBlur: onBlurHandler,
    style: _extends({}, otherProps, {
      boxSizing: 'border-box',
      color: 'inherit',
      fontSize: 'inherit',
      fontFamily: "inherit",
      backgroundColor: "inherit",
      width: '100%'
    })
  }, otherProps))));
};

function _templateObject() {
  var data = _taggedTemplateLiteralLoose(["\n  @font-face {\n    font-family: \"Roboto\";\n    src: url(", ")\n  }\n"]);

  _templateObject = function _templateObject() {
    return data;
  };

  return data;
}
var GlobalStyle = createGlobalStyle(_templateObject(), robotoFont);
var FormInput = styled(Input)(function (props) {
  return {
    '& input': {
      '&::placeholder': {
        color: 'inherit'
      },
      '&:focus': {
        outline: 0
      },
      backgroundColor: 'transparent',
      border: '0',
      padding: '0'
    },
    backgroundColor: '#7795f8',
    border: '1px solid #819efc',
    borderRadius: '4px',
    color: props.valid ? '#fff' : '#FFC7EE',
    fontFamily: 'inherit',
    fontSize: '1em',
    height: '1em',
    padding: '1.5em',
    width: '100%'
  };
});
var PayButton = styled.button(function (props) {
  return {
    '&:hover': {
      cursor: !props.disabled ? 'pointer' : 'not-allowed'
    },
    backgroundColor: '#f6a4eb',
    border: '1px solid #2e6da4',
    borderRadius: '4px',
    boxShadow: '0 6px 9px rgba(50, 50, 93, 0.06), 0 2px 5px rgba(0, 0, 0, 0.08), inset 0 1px 0 #ffb9f6',
    color: 'white',
    fontSize: '1.1em',
    opacity: props.disabled ? 0.9 : 1,
    fontFamily: 'inherit',
    padding: '12px 48px',
    width: '100%'
  };
});

var CardNumber = function CardNumber(_ref) {
  var _ref$type = _ref.type,
      type = _ref$type === void 0 ? 'text' : _ref$type,
      _ref$name = _ref.name,
      _ref$placeholder = _ref.placeholder,
      placeholder = _ref$placeholder === void 0 ? '1234 5678 9012 3456' : _ref$placeholder,
      props = _objectWithoutPropertiesLoose(_ref, ["type", "name", "placeholder"]);

  return React.createElement(FormInput, _extends({
    name: "cardNumber",
    type: type,
    placeholder: placeholder,
    guide: false,
    label: React.createElement(FaCreditCard, {
      size: "1.1em"
    }),
    mask: [/\d/, /\d/, /\d/, /\d/, ' ', /\d/, /\d/, /\d/, /\d/, ' ', /\d/, /\d/, /\d/, /\d/, ' ', /\d/, /\d/, /\d/, /\d/]
  }, props));
};

var CardCode = function CardCode(_ref2) {
  var _ref2$type = _ref2.type,
      type = _ref2$type === void 0 ? 'text' : _ref2$type,
      _ref2$name = _ref2.name,
      name = _ref2$name === void 0 ? 'cardCode' : _ref2$name,
      _ref2$placeholder = _ref2.placeholder,
      placeholder = _ref2$placeholder === void 0 ? 'CVC' : _ref2$placeholder,
      props = _objectWithoutPropertiesLoose(_ref2, ["type", "name", "placeholder"]);

  return React.createElement(FormInput, _extends({
    name: name,
    type: type,
    placeholder: placeholder,
    guide: false,
    mask: [/\d/, /\d/, /\d/, /\d?/]
  }, props));
};

var ExpDate = function ExpDate(_ref3) {
  var _ref3$type = _ref3.type,
      type = _ref3$type === void 0 ? 'text' : _ref3$type,
      _ref3$name = _ref3.name,
      name = _ref3$name === void 0 ? 'expDate' : _ref3$name,
      _ref3$placeholder = _ref3.placeholder,
      placeholder = _ref3$placeholder === void 0 ? 'MM/YY' : _ref3$placeholder,
      props = _objectWithoutPropertiesLoose(_ref3, ["type", "name", "placeholder"]);

  return React.createElement(FormInput, _extends({
    name: name,
    type: type,
    placeholder: placeholder,
    mask: [/\d/, /\d/, '/', /\d/, /\d/],
    guide: false,
    label: React.createElement(FaRegCalendarAlt, {
      size: "1.1em"
    })
  }, props));
};

var ErrorComponent = function ErrorComponent(props) {
  var slugs = {
    cardCode: 'Card code',
    cardNumber: 'Card number',
    expDate: 'Expiration date'
  };
  return React.createElement(Flex, {
    style: props.style,
    key: props.field,
    justifyContent: "center",
    alignItems: "center"
  }, React.createElement(FaExclamationCircle, {
    color: "inherit"
  }), React.createElement(Text, {
    fontFamily: "inherit",
    color: "inherit",
    pl: 2
  }, slugs[props.field], " is not valid"));
};

var FormComponent = function FormComponent(_ref4) {
  var style = _ref4.style,
      props = _objectWithoutPropertiesLoose(_ref4, ["style"]);

  var canSubmit = values(props.validationErrors).every(function (value) {
    return value === true;
  });
  var invalidFields = toPairs(props.validationErrors).filter(function (_ref5) {
    var status = _ref5[1];
    return !status;
  }).map(function (_ref6) {
    var v = _ref6[0];
    return v;
  }).slice(0, 1);
  return React.createElement(Flex, {
    className: props.className,
    style: style && style.form,
    flexWrap: "wrap",
    mb: 4,
    bg: "#6772e5",
    p: [3, 5],
    justifyContent: "center"
  }, React.createElement(GlobalStyle, null), React.createElement(Box, {
    width: [1, 2 / 4],
    mb: [3, 0]
  }, React.createElement(CardNumber, {
    style: style && style.input,
    onFocus: curry(props.handleFocus)('cardNumber'),
    onBlur: props.handleBlur,
    onChange: curry(props.handleChange)('cardNumber'),
    focused: props.focused === 'cardNumber',
    valid: props.validationErrors.cardNumber,
    value: props.values.cardNumber
  })), React.createElement(Box, {
    width: [1 / 2, 1 / 4],
    pl: [0, 4]
  }, React.createElement(CardCode, {
    style: style && style.input,
    onFocus: curry(props.handleFocus)('cardCode'),
    onBlur: props.handleBlur,
    onChange: curry(props.handleChange)('cardCode'),
    focused: props.focused === 'cardCode',
    valid: props.validationErrors.cardCode,
    value: props.values.cardCode
  })), React.createElement(Box, {
    width: [1 / 2, 1 / 4],
    pl: 2
  }, React.createElement(ExpDate, {
    style: style && style.input,
    onFocus: curry(props.handleFocus)('expDate'),
    onBlur: props.handleBlur,
    onChange: curry(props.handleChange)('expDate'),
    valid: props.validationErrors.expDate,
    focused: props.focused === 'expDate',
    value: props.values.expDate
  })), React.createElement(Box, {
    width: [1, 1 / 2],
    pt: 4
  }, React.createElement(PayButton, {
    style: style && style.button,
    disabled: !canSubmit,
    onClick: canSubmit ? props.handleSubmit : undefined
  }, "Pay $", props.amount)), React.createElement(Box, {
    width: 1,
    py: 4
  }, React.createElement(Motion, {
    key: head(invalidFields),
    style: {
      opacity: spring(1)
    },
    defaultStyle: {
      opacity: 0
    }
  }, function (config) {
    return React.createElement("div", {
      style: config
    }, head(invalidFields) ? React.createElement(ErrorComponent, {
      key: head(invalidFields),
      field: head(invalidFields)
    }) : null);
  })));
};

var FormComponent$1 = styled(FormComponent)({
  fontFamily: 'system-ui'
});

var dispatchData =
/*#__PURE__*/
function () {
  var _ref = _asyncToGenerator(
  /*#__PURE__*/
  _regeneratorRuntime.mark(function _callee(payload) {
    return _regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            return _context.abrupt("return", new Promise(function (resolve, reject) {
              return Accept.dispatchData(payload, function (response) {
                switch (response.messages.resultCode) {
                  case 'Ok':
                    resolve(response);
                    break;

                  case 'Error':
                    reject(response);
                    break;
                }
              });
            }));

          case 1:
          case "end":
            return _context.stop();
        }
      }
    }, _callee);
  }));

  return function dispatchData(_x) {
    return _ref.apply(this, arguments);
  };
}();

var Accept$1 = {
  dispatchData: dispatchData
};

var validateCardCode = function validateCardCode(input) {
  return RegExp(/^\d{3}\d?$/).test(input);
};
var validateCardNumber = function validateCardNumber(input) {
  return RegExp(/^(?:4[0-9]{12}(?:[0-9]{3})?|(?:5[1-5][0-9]{2}|222[1-9]|22[3-9][0-9]|2[3-6][0-9]{2}|27[01][0-9]|2720)[0-9]{12}|3[47][0-9]{13}|3(?:0[0-5]|[68][0-9])[0-9]{11}|6(?:011|5[0-9]{2})[0-9]{12}|(?:2131|1800|35\d{3})\d{11})$/).test(input);
};
var validateExpDate = function validateExpDate(input) {
  return RegExp(/^\d\d\/\d\d$/).test(input);
};

var AuthorizeNetScriptUrl;

(function (AuthorizeNetScriptUrl) {
  AuthorizeNetScriptUrl["Production"] = "https://js.authorize.net/v1/Accept.js";
  AuthorizeNetScriptUrl["Sandbox"] = "https://jstest.authorize.net/v1/Accept.js";
})(AuthorizeNetScriptUrl || (AuthorizeNetScriptUrl = {}));

var FormContainer =
/*#__PURE__*/
function (_React$Component) {
  _inheritsLoose(FormContainer, _React$Component);

  function FormContainer(props) {
    var _this;

    _this = _React$Component.call(this, props) || this;
    _this.state = _this.props.initialState || {
      apiErrors: [],
      focused: undefined,
      values: {
        cardNumber: '',
        cardCode: '',
        expDate: ''
      }
    };
    _this.submitHandler = _this.submitHandler.bind(_assertThisInitialized(_this));
    _this.changeHandler = _this.changeHandler.bind(_assertThisInitialized(_this));
    _this.blurHandler = _this.blurHandler.bind(_assertThisInitialized(_this));
    _this.focusHandler = _this.focusHandler.bind(_assertThisInitialized(_this));
    return _this;
  }

  var _proto = FormContainer.prototype;

  _proto.componentDidMount = function componentDidMount() {
    var script = document.createElement('script');

    switch (this.props.environment) {
      case 'production':
        script.src = AuthorizeNetScriptUrl.Production;
        break;

      case 'sandbox':
        script.src = AuthorizeNetScriptUrl.Sandbox;
        break;
    }

    document.head.appendChild(script);
  };

  _proto.componentWillUnmount = function componentWillUnmount() {
    var head$$1 = document.head;
    var scripts = head$$1.getElementsByTagName('script');
    Array.from(scripts).filter(function (script) {
      return script.src === AuthorizeNetScriptUrl.Production || script.src === AuthorizeNetScriptUrl.Sandbox;
    }).forEach(function (injectedScript) {
      return head$$1.removeChild(injectedScript);
    });
  };

  _proto.submitHandler = function submitHandler() {
    var _this2 = this;

    var authData = {
      apiLoginID: this.props.apiLoginId,
      clientKey: this.props.clientKey
    };

    var _this$state$values$ex = this.state.values.expDate.split('/'),
        month = _this$state$values$ex[0],
        year = _this$state$values$ex[1];

    var cardData = {
      cardCode: this.state.values.cardCode,
      cardNumber: this.state.values.cardNumber.replace(/\s/g, ''),
      month: month,
      year: year
    };
    var secureData = {
      authData: authData,
      cardData: cardData
    };
    return Accept$1.dispatchData(secureData).then(function (response) {
      if (_this2.props.onSuccess) {
        _this2.props.onSuccess(response, _this2.state.values);
      }

      _this2.setState({
        values: {
          cardCode: '',
          cardNumber: '',
          expDate: ''
        }
      });

      return response;
    })["catch"](function (response) {
      _this2.setState({
        apiErrors: response.messages.message.map(function (err) {
          return err.text;
        })
      });

      if (_this2.props.onError) {
        _this2.props.onError(response);
      }

      throw response;
    });
  };

  _proto.changeHandler = function changeHandler(field, ev) {
    this.setState(function (oldState) {
      var _extends2;

      return _extends({}, oldState, {
        values: _extends({}, oldState.values, (_extends2 = {}, _extends2[field] = ev.target.value, _extends2))
      });
    });
    return ev;
  };

  _proto.focusHandler = function focusHandler(field, ev) {
    this.setState({
      focused: field
    });
    return ev;
  };

  _proto.blurHandler = function blurHandler(ev) {
    this.setState({
      focused: undefined
    });
    return ev;
  };

  _proto.render = function render() {
    var View = this.props.render || this.props.component || this.props.children;
    return View ? React.createElement("div", null, React.createElement(View, {
      amount: this.props.amount,
      apiErrors: this.state.apiErrors,
      focused: this.state.focused,
      handleChange: this.changeHandler,
      handleFocus: this.focusHandler,
      handleBlur: this.blurHandler,
      handleSubmit: this.submitHandler,
      validationErrors: FormContainer.runValidations(pick(['cardCode', 'cardNumber', 'expDate'], this.state.values)),
      values: pick(['cardCode', 'cardNumber', 'expDate'], this.state.values)
    })) : null;
  };

  return FormContainer;
}(React.Component);

FormContainer.runValidations = function (formValues) {
  return {
    cardCode: validateCardCode(formValues.cardCode),
    cardNumber: validateCardNumber(formValues.cardNumber.replace(/\s/g, '')),
    expDate: validateExpDate(formValues.expDate)
  };
};
FormContainer.propTypes = process.env.NODE_ENV !== "production" ? {
  amount: PropTypes.number,
  apiLoginId: PropTypes.string.isRequired,
  children: PropTypes.func,
  clientKey: PropTypes.string.isRequired,
  component: PropTypes.any,
  environment: PropTypes.oneOf(['sandbox', 'production']).isRequired,
  render: PropTypes.func
} : {};

function isValidMessage(o) {
  return typeof o === 'object' && o !== null;
}

function isValidTransaction(o) {
  return o !== null && o !== undefined && typeof o === 'object';
}

var AuthorizeNetPostUrl = {
  production: 'https://accept.authorize.net/payment/payment',
  sandbox: 'https://test.authorize.net/payment/payment'
};
var AcceptHosted = function AcceptHosted(_ref) {
  var formToken = _ref.formToken,
      _ref$mode = _ref.mode,
      mode = _ref$mode === void 0 ? 'sandbox' : _ref$mode,
      _ref$type = _ref.type,
      type = _ref$type === void 0 ? 'iframe' : _ref$type,
      onMessage = _ref.onMessage,
      style = _ref.style,
      className = _ref.className,
      children = _ref.children,
      _onReceiveCommunication = _ref.onReceiveCommunication,
      onCancel = _ref.onCancel,
      onResize = _ref.onResize,
      onTransact = _ref.onTransact;
  var formTrigger = children ? children : React.createElement("button", null, "Pay");
  useEffect(function () {
    window.AuthorizeNetIFrame = {
      onReceiveCommunication: function onReceiveCommunication(queryStr) {
        _onReceiveCommunication && _onReceiveCommunication(queryStr);
        var message = parse(queryStr);

        if (isValidMessage(message)) {
          onMessage && onMessage(message);

          switch (message.action) {
            case 'transactResponse':
              var json = JSON.parse(message.response);

              if (isValidTransaction(json)) {
                onTransact && onTransact(json);
              }

              break;

            case 'resizeWindow':
              onResize && onResize(message.width, message.height);
              break;

            case 'cancel':
              onCancel && onCancel();
              break;
          }
        }
      }
    };
    return function () {
      delete window.AuthorizeNetIFrame;
    };
  }, []);

  var _useState = useState(false),
      submitted = _useState[0],
      setSubmitted = _useState[1];

  switch (type) {
    case 'iframe':
      return React.createElement("div", {
        style: style,
        className: className
      }, React.createElement("form", {
        onSubmit: function onSubmit() {
          return setSubmitted(true);
        },
        method: 'POST',
        action: AuthorizeNetPostUrl[mode],
        target: 'iframeAuthorizeNet'
      }, React.createElement("input", {
        name: "token",
        type: 'hidden',
        value: formToken
      }), !submitted && formTrigger), React.createElement("iframe", {
        id: 'iframeAuthorizeNet',
        name: 'iframeAuthorizeNet',
        frameBorder: 0,
        width: '100%'
      }));

    case 'redirect':
      return React.createElement("form", {
        style: style,
        className: className,
        method: 'POST',
        action: AuthorizeNetPostUrl[mode]
      }, React.createElement("input", {
        type: 'hidden',
        name: 'token',
        value: formToken
      }), formTrigger);
  }
};

export { FormContainer, FormComponent$1 as FormComponent, AcceptHosted };
//# sourceMappingURL=react-authorize-net.js.map
